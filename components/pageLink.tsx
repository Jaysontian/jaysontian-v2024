...
export default function pageLink(props : any){
    return(
        <Link href={`/projects/${props.id}`} prefetch={true} >
            <div className='flex justify-between items-center text-sm py-1.5 px-1.5 hover:text-prim transition hover:bg-soft100 rounded'>
                ...
                <p className='text-xs text-soft100'>{props.desc}</p>
            </div>
        </Link>
    )
